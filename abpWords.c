...
// get words that begins with some prefix
void searchPrefix(NODE* root, const char* prefix) {
    if (root == NULL) {
        return;
    }

    int cmp = strncmp(prefix, root->word, strlen(prefix));
...
// remove all word appearances in the tree
void removeAllOccurrences(NODE* alphabet[], const char* word) {
    NODE* node = searchWord(alphabet[getHashPosition(word)], word);

    while (node != NULL) {
        if (node->timesAppears > 1) {
            node->timesAppears--;
            break;
        }

        if (node->left == NULL && node->right == NULL) {
            removeLeaf(alphabet, node);
            break;
        } else if (node->left != NULL && node->right != NULL) {
            removeChildren(alphabet, node);
        } else {
            removeChild(alphabet, node);
        }

        node = searchWord(alphabet[getHashPosition(word)], word);
    }
}
...
void showMenu(){
	...
	printf("12 -> Exibir a altura da arvore\n");
	printf("13 -> Consultar palavra por prefixo\n");
	printf("14 -> Remover todas as ocorrencias de uma palavra\n");
	printf(" 0 -> Sair\n");
	printf("\nOpcao: ");
}
...
int main(){
	...
	do{
		...
		switch(opc){
			...
			case 13:
				getchar();
				printf("Digite o prefixo para consultar: ");
				gets(word);
				searchPrefix(alphabet, word);
				break;
			case 14:
				getchar();
				printf("Digite a palavra para remover todas as ocorrencias: ");
				gets(word);
				removeAllOccurrences(alphabet, word);
				break;
			default:
				break;
		}
